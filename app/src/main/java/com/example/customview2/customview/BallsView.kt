...
class BallsView @JvmOverloads constructor(
    ...
    companion object {
        private const val DEFAULT_RADIUS = 30F // default
        const val TIME_TO_RETURN = 200L
        const val TIME_DELAY_AUTO = 300L
        const val DELTA_AUTO = 10
        const val SUM_BALLS_DEFAULT = 5
        ...
        when (event?.action) {
            ...
            MotionEvent.ACTION_MOVE -> {
                for (i in listBalls.size - 1 downTo 1) {
                    if (abs(listBalls[i].x - listBalls[i - 1].x) > 5) {
                        listBalls[i].x = listBalls[i - 1].x
                    }
                    if (abs(listBalls[i].y - listBalls[i - 1].y) > 5) {
            ...
            MotionEvent.ACTION_UP -> {
                ...
                for (i in 0 until listBalls.size) {
                    val x =
                        initCoordinatesX - (2 * i + 1) * (initCoordinatesX / (2 * sumBall)).toFloat()
                    val y = initCoordinatesY / 2.toFloat()
                    ballMoveBack(i, x, y)
                }
    ...
    inner class DelayThread : Runnable {
        override fun run() {
            val x =
                initCoordinatesX - (2 * (sumBall - 1) + 1) * (initCoordinatesX / (2 * sumBall)).toFloat()
            val y = initCoordinatesY / 2.toFloat()
